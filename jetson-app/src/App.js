...
import { isLocal, getRosbridgeUrl, apiStop, apiConfig } from './api';
...
function App() {
  ...
  // Customer/device IDs come from env vars on the Jetson — fetch once per host
  // and hold at App level so tab switches don't reset them.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
  ...
  // Track recording transitions and tag the logger context with active patrol/recording.
  ...
  useEffect(() => {
    ...
    if (wasActive && !isActive) {
      // Use lastIds so the stop event itself carries the right tags.
      ...
      // Clear after the stop event so subsequent events aren't mis-tagged.
  ...
  // Stop from the global indicator. Mirrors the Recording-tab stop flow:
  // publish stop on /recording/command, kill the sensor recorder tmux session.
  const stopFromIndicator = async () => {
    ...
    try {
      ...
      // Pending clears automatically when /recording/status reports idle.
    ...
    <div style={styles.app}>
      ...
      <div style={styles.content}>
        <RecordingStatusBar
          rec={rec}
          jetsonConnected={jetsonConnected}
          onStop={stopFromIndicator}
          onGoToRecordingTab={goToRecordingTab}
          onView={view}
        />
        {view === 'jetson' ? (
          <JetsonView
            ros={jetsonRos}
            connected={jetsonConnected}
            jetsonHost={jetsonHost}
            domainId={domainId}
            tab={jetsonTab}
            onTabChange={setJetsonTab}
            rec={rec}
            identity={identity}
          />
        ) : (
...
// Lightweight wrapper that owns LocalView's legacy local-only recording state.
// LocalView controls a separate /recording/command flow used only in the desktop view.
    ...
    <div style={styles.statusBar(isActive)}>
      ...
      <div style={styles.statusRight}>
        ...
        {isActive ? (
          ...
          jetsonConnected && (
            <button style={styles.goBtn} onClick={onGoToRecordingTab}>
              ● Go to Recording
            </button>
          )
        )}
...
const styles = {
  ...
  recDot: (active) => ({
    ...
    animation: active ? 'recPulse 1.6s ease-out infinite' : 'none',
    flexShrink: 0,
  }),
  recLabel: (active) => ({
  ...
  elapsed: {
    fontSize: 18, fontWeight: 700, color: '#ff9f43',
    fontVariantNumeric: 'tabular-nums', letterSpacing: '1px',
    paddingLeft: 8, borderLeft: '1px solid #ff475733', flexShrink: 0,
  },
  frames: {
    fontSize: 11, color: '#aaa', fontWeight: 600, flexShrink: 0,
  },
  idsBlock: {
    display: 'flex', alignItems: 'center', gap: 6,
    paddingLeft: 12, marginLeft: 4, borderLeft: '1px solid #2a1518',
    fontFamily: 'monospace', minWidth: 0, overflow: 'hidden',
  },
  ...
  goBtn: {
    ...
    background: '#6c5ce715', border: '1px solid #6c5ce744', color: '#6c5ce7',
