  ...
  const toggleRecording = async () => {
    if (recording) {
      ...
      if (enabledSensorTopics.length > 0) {
        try { await apiStop('mcap_recorder'); } catch {}
      }
    ...
    if (!overwriteWarn) {
      setChecking(true);
      try {
        const d = await apiExec(`test -d /AgroTech_recordings/${companyId}/${deviceId}/${patrolId}/${recordingId} && echo EXISTS || echo NO`, jetsonHost);
    ...
    if (enabledSensorTopics.length > 0) {
      ...
      try {
        await apiLaunch('mcap_recorder', `${rosEnv(domainId)} && python3 /home/ada/mcap_recorder.py --company "${companyId}" --device "${deviceId}" --patrol "${patrolId}" --recording "${recordingId}" --topics "${topicsArg}"`, jetsonHost);
      } catch {}
