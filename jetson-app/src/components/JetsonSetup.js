...
import { apiExec, apiLaunch, apiStop, rosEnv } from '../api';
...
const useSshSession = (id, jetsonHost, command) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await apiExec(`tmux has-session -t ${id} 2>/dev/null && echo ALIVE || echo DEAD`, jetsonHost);
  ...
  const launch = async () => {
    ...
    try {
      const data = await apiLaunch(id, command, jetsonHost);
      setStatus(data.ok ? 'running' : 'error');
      setOutput(data.message || data.error || '');
    } catch (e) { setStatus('error'); setOutput(`Cannot reach bridge: ${e.message}`); }
  };
  const stop = async () => {
    setStatus('stopping');
    try {
      await apiStop(id, jetsonHost);
...
export const JetsonSetup = ({ jetsonHost, connected, domainId }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const d = await apiExec(`${rosEnv(domainId)} && python3 /home/ada/calibrate_headless.py --list-profiles`, jetsonHost);
        if (d.ok) {
