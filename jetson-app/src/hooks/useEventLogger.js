...
// Singleton flush state — survives multiple hook callers without losing buffered events.
const buffer = [];
let flushTimer = null;
let inFlight = false;
...
// Per-call defaults that auto-tag every event with whatever recording is active.
let context = { customer: null, device: null, patrol: null, recording: null };

export function setLoggerContext(next) {
  context = { ...context, ...next };
}

export function clearLoggerContext(keys) {
  for (const k of keys) context[k] = null;
}
...
const drainAndFlush = async (host) => {
  ...
  try {
    ...
    // Re-queue (bounded). Drop oldest if we're way behind.
...
export function logEvent(level, event, meta, opts = {}) {
  ...
  buffer.push(e);
  while (buffer.length > MAX_BUFFER) buffer.shift();
  // Errors flush immediately; everything else batches.
  if (level === 'error' && opts.host) drainAndFlush(opts.host);
  else if (opts.host) scheduleFlush(opts.host);
}

// React hook: keeps the host ref current, installs global error handlers, periodic flush.
export function useEventLogger(host) {
  const hostRef = useRef(host);
  hostRef.current = host;

  // Sticky periodic flush — handles the case where `logEvent` was called before the host was known.
  useEffect(() => {
    const iv = setInterval(() => { if (hostRef.current) drainAndFlush(hostRef.current); }, FLUSH_INTERVAL_MS);
    return () => clearInterval(iv);
  }, []);

  // Capture window-level crashes so we can audit the React black-screen class of bugs.
  useEffect(() => {
    const onError = (e) => {
  ...
  // Stable callback for components.
